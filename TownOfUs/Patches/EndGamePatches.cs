...
public static class EndGamePatches
{
    public static void BuildEndGameData()
    {
        ...
        foreach (var playerStats in GameHistory.PlayerStats.Values)
        {
            ...
            if (playerStats.IsSpectator)
            {
                ...
                {
                    ChatSummaryTitle =
                        $"{playerStats.PlayerName} - {MiscUtils.GetMaskedRoleTmpIcon((RoleTypes)RoleId.Get<SpectatorRole>())}{MiraLocaleManager.Get("TownOfUsMira.Role.Spectator")}",
                    ChatSummaryRoleInfo = string.Empty,
                    ChatSummaryStats = string.Empty,
                    ChatSummaryCod = string.Empty,
                    PlayerName = playerStats.PlayerName,
                    RoleString = MiscUtils.GetMaskedRoleTmpIcon((RoleTypes)RoleId.Get<SpectatorRole>()) +
                                 MiraLocaleManager.Get("TownOfUsMira.Role.Spectator"),
                    RoleStringShort = MiscUtils.GetMaskedRoleTmpIcon((RoleTypes)RoleId.Get<SpectatorRole>()) +
                                      MiraLocaleManager.Get("TownOfUsMira.Role.Spectator"),
                    Winner = false,
            ...
            foreach (var role in playerStats.TrackedRoles)
            {
                ...
                {
                    roleName = TranslationController.Instance.GetString(
                ...
                playerRoleString.Append(
                    ...
                    $"{color.ToTextColor()}{roleName}</color> > ");
            }

            if (playerRoleString.Length > 3)
            {
                playerRoleString = playerRoleString.Remove(
                    playerRoleString.Length - 3,
                    3);
            }
            ...
            var modifiers = playerStats.LastKnownModifiers
                .Where(x =>
                    x is TouGameModifier touMod && touMod.AppearsInSummary ||
                    x is UniversalGameModifier);
            ...
            if (playerStats.DisplayedRole is IProgressTally tally)
            {
                ...
                if (tally.ProgressOnSummaryDetailed != string.Empty)
                {
                    summaryStats.Append(
                        TownOfUsPlugin.Culture,
                        $" | {tally.ProgressOnSummaryDetailed}");
                }
            }
            else if (playerTeam == ModdedRoleTeams.Crewmate &&
                     playerStats.PlayerState != StoredPlayerState.Disconnected)
            {
                var playerControl = PlayerControl.AllPlayerControls
                    .ToArray()
                    .FirstOrDefault(x => x.PlayerId == playerStats.PlayerId);
                ...
                if (playerControl != null)
                {
                    ...
                    summaryStats.Append(
                        TownOfUsPlugin.Culture,
                        $" | {MiraLocaleManager.Get("StatsTaskCount").Replace("<count>", taskInfo.Replace("(", "").Replace(")", ""))}");
                }
            }

            var killedPlayers = GameHistory.KilledPlayers.Count(x =>
                x.KillerId == playerStats.PlayerId &&
                x.VictimId != playerStats.PlayerId);

            if (GameHistory.PlayerStats.TryGetValue(
                    ...
                    out var stats))
            {
                ...
                else if (basicKillCount > 0 &&
                         !playerStats.DisplayedRole.IsCrewmate())
                {
                    summaryStats.Append(
                        TownOfUsPlugin.Culture,
            ...
            else if (killedPlayers > 0 &&
                     !playerStats.DisplayedRole.IsCrewmate() &&
                     playerStats.DisplayedRole.GetRoleAlignment() !=
            ...
            var hasExtendedCauseOfDeath =
                !string.IsNullOrEmpty(playerStats.ExtendedCauseOfDeath);
            ...
            if (!hasExtendedCauseOfDeath &&
                playerStats.KilledBy != string.Empty)
            {
                playerRoleString.Append(
                    TownOfUsPlugin.Culture,
            ...
            if (EndGameResult.CachedWinners
                .ToArray()
                .Any(x => x.PlayerName == playerStats.PlayerName))
            {
                playerName.Append(
                    TownOfUsPlugin.Culture,
                    $"<color=#EFBF04>{playerStats.PlayerName}</color>");
            ...
            summaryTitle.Append(
                TownOfUsPlugin.Culture,
                $"{playerName} - {latestRole}{modifierHolder}");

            var alliance = playerStats.LastKnownModifiers
                .OfType<AllianceGameModifier>()
                .FirstOrDefault();
            ...
            EndGameData.PlayerRecords.Add(
                new EndGameData.PlayerRecord
                {
    ...
    public static void BuildEndGameSummary(EndGameManager instance)
    {
        ...
        roleSummaryLeft.transform.position =
            new Vector3(
                exitBtn.transform.position.x + 0.1f,
                position.y - 0.1f,
                -14f);

        roleSummaryLeft.transform.localScale =
        ...
        roleSummary.transform.localScale =
            new Vector3(1f, 1f, 1f);

        var roleSummary2 = Object.Instantiate(winText.gameObject);

        roleSummary2.transform.position =
            new Vector3(
                exitBtn.transform.position.x + 0.1f,
                position.y - 0.1f,
                -14f);

        roleSummary2.transform.localScale =
            new Vector3(1f, 1f, 1f);
        ...
        var summaryTxt =
            MiraLocaleManager.Get("EndGameSummary") + ":";
        ...
        foreach (var data in EndGameData.PlayerRecords)
        {
            ...
            if (count % 2 == 0)
            {
                roleSummaryText2.AppendLine(
                    TownOfUsPlugin.Culture,
                    $"{data.PlayerName} - {role2}");
            }
            else
            {
            ...
            normalSummary.AppendLine(
                TownOfUsPlugin.Culture,
                $"<size=62%>{data.PlayerName} - {role}");

            basicSummary.AppendLine(
                TownOfUsPlugin.Culture,
                $"<size=62%>{data.PlayerName} - {role2}");

            segmentedSummary.AppendLine(
                TownOfUsPlugin.Culture,
                $"<size=70%>{data.ChatSummaryTitle}</size>");

            segmentedSummary.Append(
                TownOfUsPlugin.Culture,
                $"<size=62%>");
        ...
        var roleSummaryTextMesh =
            roleSummary.GetComponent<TMP_Text>();

        roleSummaryTextMesh.alignment =
            TextAlignmentOptions.TopLeft;

        roleSummaryTextMesh.color =
            Color.white;
        ...
        var roleSummaryTextMesh2 =
            roleSummary2.GetComponent<TMP_Text>();

        roleSummaryTextMesh2.alignment =
            TextAlignmentOptions.TopLeft;

        roleSummaryTextMesh2.color =
            Color.white;
        ...
        var roleSummaryTextMeshLeft =
            roleSummaryLeft.GetComponent<TMP_Text>();

        roleSummaryTextMeshLeft.alignment =
            TextAlignmentOptions.TopLeft;

        roleSummaryTextMeshLeft.color =
            Color.white;
        ...
        {
            roleSummaryTextMesh.font =
                controllerHandler.ContinueText.GetComponent<TMP_Text>().font;

            roleSummaryTextMesh.fontStyle =
                FontStyles.Bold;
        } */

        var roleSummaryTextMeshRectTransform =
            roleSummaryTextMesh.GetComponent<RectTransform>();
        ...
        roleSummaryTextMesh.text =
            roleSummaryText1
                .ToString()
                .Replace(".Masked", string.Empty);

        var roleSummaryTextMeshRectTransform2 =
            roleSummaryTextMesh2.GetComponent<RectTransform>();

        roleSummaryTextMeshRectTransform2.anchoredPosition =
            new Vector2(
                position.x + 8.8f,
                position.y - 0.1f);

        roleSummaryTextMesh2.text =
            roleSummaryText2
                .ToString()
                .Replace(".Masked", string.Empty);

        var roleSummaryTextMeshRectTransformLeft =
            roleSummaryTextMeshLeft.GetComponent<RectTransform>();

        roleSummaryTextMeshRectTransformLeft.anchoredPosition =
            new Vector2(
                position.x + 3.5f,
                position.y - 0.1f);

        roleSummaryTextMeshLeft.text =
            roleSummaryTextFull
                .ToString()
                .Replace(".Masked", string.Empty);

        GameHistory.EndGameSummarySimple =
            basicSummary.ToString();

        GameHistory.EndGameSummary =
            normalSummary.ToString();

        GameHistory.EndGameSummaryAdvanced =
            segmentedSummary.ToString();

        var GameSummaryButton =
            Object.Instantiate(exitBtn);

        GameSummaryButton.gameObject.SetActive(true);

        GameSummaryButton.sprite =
            TouAssets.GameSummarySprite.LoadAsset();

        GameSummaryButton.transform.position +=
            Vector3.up * 1.65f;

        if (GameSummaryButton.transform
            .GetChild(1)
            .TryGetComponent<TextTranslatorTMP>(out var tmp2))
        {
            var text =
                MiraLocaleManager
                    .Get("GameSummaryModeButton")
                    .Split(":");

            if (text.Length == 1 ||
                text.Any(x => x == string.Empty))
            {
        ...
        switch (LocalSettingsTabSingleton<TouLocalTabPreferences>
                    .Instance
                    .EndSummaryVisibility
                    .Value)
        {
            default:
                // No summary
                ...
                LocalSettingsTabSingleton<TouLocalTabPreferences>
                    .Instance
                    .EndSummaryVisibility
                    .Value =
                    EndGameSummaryVisibility.Hidden;
                        ...
                        .Value)
            {
                case EndGameSummaryVisibility.Hidden:
                    ...
                    LocalSettingsTabSingleton<TouLocalTabPreferences>
                        ...
                        .EndSummaryVisibility
                        .Value =
                        EndGameSummaryVisibility.Split;
                ...
                case EndGameSummaryVisibility.Split:
                    ...
                    LocalSettingsTabSingleton<TouLocalTabPreferences>
                        ...
                        .EndSummaryVisibility
                        .Value =
                        EndGameSummaryVisibility.LeftSide;
        ...
        var passiveButton =
            GameSummaryButton.GetComponent<PassiveButton>();
            ...
            foreach (var player in array)
            {
                ...
                realPlayer ??=
                    winnerArray.FirstOrDefault(
                        x =>
                            x.Outfit.HatId ==
                            player.cosmetics.hat.Hat.ProdId &&
                            x.Outfit.ColorId ==
                ...
                var actualRole =
                    RoleManager.Instance.GetRole(
                        realPlayer.RoleWhenAlive);
                ...
                {
                    foreach (var role in GameHistory.RoleHistory
                                 .Where(
                                     x => x.Key ==
                                          realDealPlayer.PlayerId)
                                 .Select(x => x.Value))
                    {
                        if (role.Role is
                            ...
                            role.Role ==
                            (RoleTypes)RoleId.Get<NeutralGhostRole>())
                        {
                            continue;
                        }
                ...
                // The winner-name TMP is not rendered through a masked UI element.
                // Strip the ".Masked" suffix from the TMP sprite name here only.
                var roleIcon =
                    MiscUtils
                        .GetMaskedRoleTmpIcon(actualRole)
                        .Replace(".Masked", string.Empty);

                player.SetName(
                    $"\n<size=85%>{realPlayer.PlayerName}</size>\n<size=65%><color=#{actualRole.TeamColor.ToHtmlStringRGBA()}>{roleIcon}{actualRole.GetRoleName()}</color></size>",
                    new Vector3(1.1619f, 1.1619f, 1f),
                    Color.white,
                    -15f);

                player.SetNamePosition(
                    new Vector3(
                        0f,
                        -1.31f,
                        -0.5f));
    ...
    public static void AfterEndGameSetup(
        EndGameManager instance)
    {
        var text =
            Object.Instantiate(instance.WinText);

        switch (EndGameEvents.winType)
        {
            case 1:
                text.text =
                    $"<size=4>{MiraLocaleManager.Get("CrewmatesWin")}!</size>";

                text.color =
                    Palette.CrewmateBlue;

                instance.BackgroundBar.material.SetColor(
                    ShaderID.Color,
                    Palette.CrewmateBlue);
            ...
            case 2:
                text.text =
                    $"<size=4>{MiraLocaleManager.Get("ImpostorsWin")}!</size>";
        ...
        var pos =
            instance.WinText.transform.localPosition;
    ...
    [HarmonyPatch(
        ...
        nameof(AmongUsClient.OnGameEnd))]
    [HarmonyPostfix]
    public static void AmongUsClientGameEndPatch()
    {
        ...
        {
            foreach (var log in
                     TownOfUsEventHandlers.LogBuffer)
            {
                var text = log.Value;
    ...
    public static class EndGameData
    {
        ...
        {
            public string? ChatSummaryTitle
            {
                get;
                init;
            }

            public string? ChatSummaryRoleInfo
            {
                get;
                init;
            }

            public string? ChatSummaryStats
            {
                get;
                init;
            }

            public string? ChatSummaryCod
            {
                get;
                init;
            }
    ...
    {
        public static List<PlayerMeetingRecord> PlayerMeetingRecords
        {
            get;
            set;
        } = [];

        private static string GetCauseOfDeathString(
            string parsedData)
        {
            var curRound =
            ...
            return
                $"<size=60%>『{Color.yellow.ToTextColor()}{parsedData.Replace("<round>", $"{curRound}")}</color>』</size>";
        }

        public static void AddPlayerData(
            PlayerControl player)
        {
            if (PlayerMeetingRecords.Any(
                    x => x.PlayerId ==
            ...
            var state =
                GameHistory.PlayerStats[player.PlayerId];

            Warning(
                $"Added Meeting Record for {player.Data.PlayerName}");
            ...
            var taskOpt =
                OptionGroupSingleton<PostmortemOptions>
                    .Instance;

            var roleNameSize =
                HudManagerPatches.RoleIsSmall
                    ? "80%"
                    : "100%";

            var roleOnTop =
                HudManagerPatches.RoleOnTop;
            ...
            var localImp =
                ...
                genOpt is
                {
                    ImpsKnowRoles.Value: true,
                    FFAImpostorMode: false
                };

            var localVamp =
                PlayerControl.LocalPlayer
                    .GetRoleWhenAlive() is VampireRole;
            ...
            var (playerColorColored, playerNameColored) =
                GetRoleNameText(
                    player,
                    genOpt.FFAImpostorMode,
                    taskOpt,
            ...
            playerNameColored =
                playerNameColored.Replace(
                    "<cod>",
                    causeOfDeath);

            var (playerColorFull, playerNameFull) =
                GetRoleNameText(
                    player,
                    genOpt.FFAImpostorMode,
                    taskOpt,
            ...
            playerNameFull =
                playerNameFull.Replace(
                    "<cod>",
                    causeOfDeathFull);

            var (
                playerColorColoredFull,
                playerNameColoredFull) =
                GetRoleNameText(
                    player,
                    genOpt.FFAImpostorMode,
                    taskOpt,
            ...
            playerNameColoredFull =
                playerNameColoredFull.Replace(
                    "<cod>",
                    causeOfDeathFull);

            PlayerMeetingRecords.Add(
                new PlayerMeetingRecord
                {
                    PlayerNameUncolored =
                        playerName,

                    PlayerColorUncolored =
                        playerColor,
                    ...
                    PlayerNameUncoloredFull =
                        playerNameFull,

                    PlayerColorUncoloredFull =
                        playerColorFull,
        ...
        public static void DisplayRecordData(
            TextMeshPro tmp,
            PlayerMeetingRecord record,
            bool color,
            bool isLocalDead)
        {
            if (color)
            {
                tmp.text =
                    isLocalDead
                        ? record.PlayerNameColoredFull
                        : record.PlayerNameColored;

                tmp.color =
                    isLocalDead
                        ? record.PlayerColorColoredFull
                        : record.PlayerColorColored;
            }
            else
            {
                tmp.text =
                    isLocalDead
                        ? record.PlayerNameUncoloredFull
                        : record.PlayerNameUncolored;

                tmp.color =
                    isLocalDead
                        ? record.PlayerColorUncoloredFull
                        : record.PlayerColorUncolored;
            }
        ...
        {
            public string PlayerNameUncolored
            {
                get;
                init;
            }

            public Color PlayerColorUncolored
            {
                get;
                init;
            }

            public string PlayerNameColored
            {
                get;
                init;
            }

            public Color PlayerColorColored
            {
                get;
                init;
            }

            public string PlayerNameUncoloredFull
            {
                get;
                init;
            }

            public Color PlayerColorUncoloredFull
            {
                get;
                init;
            }

            public string PlayerNameColoredFull
            {
                get;
                init;
            }

            public Color PlayerColorColoredFull
            {
                get;
                init;
            }
